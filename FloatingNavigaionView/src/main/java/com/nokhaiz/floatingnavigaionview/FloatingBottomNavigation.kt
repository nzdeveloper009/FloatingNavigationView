...
class FloatingBottomNavigation : FrameLayout {
    ...
    private fun setAttributeFromXml(context: Context, attrs: AttributeSet) {
        val a = context.theme.obtainStyledAttributes(
            attrs,
            R.styleable.FloatingBottomNavigation, 0, 0
        )
        try {
            a.apply {
                defaultIconColor = getColor(
                    R.styleable.FloatingBottomNavigation_mbn_defaultIconColor,
                    defaultIconColor
                )
                selectedIconColor = getColor(
                    R.styleable.FloatingBottomNavigation_mbn_selectedIconColor,
                    selectedIconColor
                )
                backgroundBottomColor = getColor(
                    R.styleable.FloatingBottomNavigation_mbn_backgroundBottomColor,
                    backgroundBottomColor
                )
                circleColor =
                    getColor(R.styleable.FloatingBottomNavigation_mbn_circleColor, circleColor)
                countTextColor =
                    getColor(
                        R.styleable.FloatingBottomNavigation_mbn_countTextColor,
                        countTextColor
                    )
                countBackgroundColor = getColor(
                    R.styleable.FloatingBottomNavigation_mbn_countBackgroundColor,
                    countBackgroundColor
                )
                rippleColor =
                    getColor(R.styleable.FloatingBottomNavigation_mbn_rippleColor, rippleColor)
                shadowColor =
                    getColor(R.styleable.FloatingBottomNavigation_mbn_shadowColor, shadowColor)

                val typeface = getString(R.styleable.FloatingBottomNavigation_mbn_countTypeface)
                if (typeface != null && typeface.isNotEmpty())
                    countTypeface = Typeface.createFromAsset(context.assets, typeface)

                hasAnimation =
                    getBoolean(R.styleable.FloatingBottomNavigation_mbn_hasAnimation, hasAnimation)
            }
        } finally {
            a.recycle()
        }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (selectedId == -1) {
            bezierView.bezierX =
                if (Build.VERSION.SDK_INT >= 24 && layoutDirection == LayoutDirection.RTL) measuredWidth + 72f.dp(
                    context
                ) else (-72f).dp(context)
        }
        if (selectedId != -1) {
